...
pt='F:/weights/triplet_loss_trained_model.pth'
...
DATA_DIR="F:/weights/"
filename ="F:/final year/train/joshi.flac"
...
np.save(DATA_DIR + '/avishkar_embeddings.npy', mean_embeddings)
...
#Audio to be tested against stored one
# filename ="F:/final year/audio/kaushik_test.flac"
# fbanks = extract_fbanks(filename)
# embeddings = get_embeddings(fbanks)
# print('shape of embeddings: {}'.format(embeddings.shape), flush=True)
# mean_embeddings = np.mean(embeddings, axis=0)
...
filename = "F:/final year/audio/jyoti_train.flac"
fbanks = extract_fbanks(filename)
embeddings = get_embeddings(fbanks)

#Audio embeddings stored while registration
stored_embeddings = np.load(DATA_DIR + '/avishk_embeddings.npy')
